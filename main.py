...
try:
    ...
    from utils.performance_logger import log_system_health
...
def _create_capture_engine(mode):
    ...
    _logger.info("Creating Scapy/Npcap capture engine on '%s'", iface)
    return CaptureEngine(mode, interface=iface)
...
def start_anomaly_detector():
    ...
    try:
        ...
        _detector = AnomalyDetector(alert_engine=alert_engine)
...
def start_cleanup_task():
    ...
    def cleanup_loop():
        ...
        from database.queries.maintenance import (
            run_full_cleanup, get_database_size_mb,
        )
        ...
        while not shutdown_event.is_set():
            try:
                ...
                if now - last_cleanup >= timedelta(hours=1):
                    ...
                    # Legacy cleanup (alerts, etc.)
                    deleted = cleanup_old_data(days_to_keep=7)
                    if deleted > 0:
                        _logger.info("Periodic cleanup: removed %d old records", deleted)

                    # Log database size for monitoring
                    try:
                        db_size_mb = get_database_size_mb()
                        if db_size_mb > 500:
                            _logger.warning("Database size: %.1f MB — consider more aggressive retention", db_size_mb)
                        else:
                            _logger.info("Database size: %.1f MB", db_size_mb)
                    except Exception:
                        pass
                ...
                if _interface_manager:
                    ...
                    if iface_name and ip_addr:
                        try:
                            ...
                            import ipaddress
                            network = str(ipaddress.IPv4Network(f"{ip_addr}/24", strict=False))
                            discovery = NetworkDiscovery(interface=iface_name, subnet=network)
...
def start_health_logging():
    """Periodically log system health and check alert thresholds (every 60s)."""
    global _health_log_thread

    if not _HAS_PRODUCTION_LOGGING:
        return False

    def _health_loop():
        while not shutdown_event.is_set():
            try:
                health = log_system_health()
                # Feed metrics to the alert manager
                alert_manager.check_thresholds(health)
            except Exception as e:
                logging.getLogger(__name__).debug("Health log error: %s", e)
            shutdown_event.wait(60)

    _health_log_thread = threading.Thread(
        target=_health_loop,
        daemon=True,
        name="HealthLogger",
    )
    _health_log_thread.start()
    return True
...
def main():
    ...
    # Run startup migrations (cleanup invalid devices, etc.)
    try:
        from database.migrations.cleanup_invalid_devices import run_startup_migrations
        ...
        if capture_started:
            ...
            try:
                ...
                if mode and mode.interface.ip_address:
                    ...
                    # Register our own MAC as known (won't trigger security alerts)
                    if mode.interface.mac_address:
                        from alerts.alert_engine import AlertEngine
                        AlertEngine.add_known_mac(mode.interface.mac_address)
            except Exception as e:
    ...
    detector_started = start_anomaly_detector()
    if detector_started:
        logger.info("Anomaly detector started")

    # Start system health monitor
    health_started = start_health_monitor()
    if health_started:
        logger.info("System health monitor started")

    # Start periodic health logging (logs resource usage every 60s + alert checks)
    if start_health_logging():
        logger.info("Periodic health logging started (60s interval)")
