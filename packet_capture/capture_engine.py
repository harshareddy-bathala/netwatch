...
class CaptureEngine(CaptureProcessorMixin):
    ...
    def start(self) -> None:
        ...
        logger.info(
            f"Starting CaptureEngine on '{self._interface}' | "
            f"mode={self._mode.get_mode_name().value} | "
            f"filter='{bpf}' | promisc={promisc}"
        )
    ...
    def stop(self, timeout: float = 5.0) -> None:
        ...
        logger.info(
            f"CaptureEngine stopped — "
            f"captured={self._packets_captured} "
    ...
    def _capture_loop(self) -> None:
        ...
        **Critical:** The ``filter`` argument MUST come from
        ...
        logger.info(
            ...
            f"iface='{self._interface}', filter='{bpf_filter}', promisc={promisc}"
        )
        ...
        def _enqueue(pkt):
            ...
            if qsize > self._queue_size * 0.8:
                if not hasattr(self, '_bp_warned') or not self._bp_warned:
                    logger.warning(
                        f"Queue at {qsize}/{self._queue_size} ({qsize*100//self._queue_size}%%) — "
            ...
            try:
                ...
                if self._packets_dropped % 5000 == 1:
                    logger.error(
                        f"Packet queue full ({self._queue_size}) — "
                        f"dropped {self._packets_dropped} packets total"
                    )
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if self._stop_event.is_set():
                    break
                logger.error(f"OS error in capture loop: {exc}")
                time.sleep(1)  # back off before retrying
            except Exception as exc:
                if self._stop_event.is_set():
                    break
                logger.exception(f"Unexpected error in capture loop: {exc}")
                time.sleep(1)
