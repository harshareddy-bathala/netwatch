...
class NetworkDiscovery:
    ...
    def __init__(self, interface: str = None, subnet: str = None):
        ...
        logger.info(f"NetworkDiscovery initialized (interface: {interface}, subnet: {self.subnet})")
    
    def _detect_subnet(self) -> Optional[str]:
        """Detect the local subnet from interface IP."""
        try:
    ...
    def arp_scan(self, subnet: str = None, timeout: float = None) -> List[Dict]:
        ...
        try:
            logger.info(f"Starting ARP scan on {target_subnet}")
            ...
            if len(discovered) != prev_count:
                ...
                logger.debug(f"ARP scan: {len(discovered)} devices (unchanged)")
    ...
    def _arp_scan_fallback(self) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"Fallback ARP scan error: {e}")
    ...
    def ping_sweep(self, subnet: str = None, max_workers: int = 50) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Starting ping sweep of {len(hosts)} hosts")
            ...
            logger.info(f"Ping sweep found {len(discovered)} responding hosts")
    ...
    def start_dhcp_monitoring(self):
        ...
        def dhcp_callback(pkt):
            try:
                ...
                logger.debug(f"DHCP monitoring error: {e}")
        ...
        try:
            ...
            logger.error(f"DHCP monitoring error: {e}")
    ...
    def extract_devices_from_traffic(self, packet) -> Optional[Dict]:
        ...
        try:
            ...
            logger.debug(f"Traffic extraction error: {e}")
            return None
    ...
    def _add_device(self, device: Dict):
        ...
        with self.discovery_lock:
            ...
            if mac in self.discovered_devices:
                ...
                logger.info(f"New device discovered: {device.get('ip', 'unknown')} - {mac}")
...
class PromiscuousModeHandler:
    ...
    def enable_promiscuous(interface: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to enable promiscuous mode: {e}")
            return False
        except Exception as e:
            logger.error(f"Error enabling promiscuous mode: {e}")
            return False
    ...
    def disable_promiscuous(interface: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error disabling promiscuous mode: {e}")
            return False
