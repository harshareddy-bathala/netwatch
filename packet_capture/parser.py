...
# MAC vendor lookup library (optional)
try:
...
def extract_ip_info(packet) -> Tuple[Optional[str], Optional[str], Optional[int]]:
    ...
    try:
        ...
        logger.debug(f"Error extracting IP info: {e}")
        return None, None, None
...
def extract_port_info(packet) -> Tuple[Optional[int], Optional[int]]:
    ...
    try:
        ...
        logger.debug(f"Error extracting port info: {e}")
        return None, None
...
def get_raw_protocol(packet) -> str:
    ...
    try:
        ...
        logger.debug(f"Error determining protocol: {e}")
        return 'UNKNOWN'
...
def resolve_device_name(mac_address: str, ip_address: str) -> Optional[str]:
    ...
    try:
        ...
        logger.debug(f"Hostname lookup failed for {ip_address}: {e}")
    
    # Method 2: Try NetBIOS name lookup (Windows only)
    try:
        import sys
        if sys.platform == 'win32':
            # Use Windows-specific NetBIOS lookup
            import subprocess
            ...
            if result.returncode == 0:
                # Parse nbtstat output for hostname
                ...
                for line in lines:
                    if '<00>' in line and 'UNIQUE' in line:
                        ...
                        if parts:
                            ...
                            if netbios_name and netbios_name != '__MSBROWSE__':
                                return netbios_name
    except Exception as e:
