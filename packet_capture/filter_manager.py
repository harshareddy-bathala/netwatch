    ...
    def get_validated_filter(self) -> str:
        ...
        if mode_name == ModeName.PORT_MIRROR:
            if not bpf:
                ...
                logger.info(f"PortMirrorMode: BPF filter = '{bpf}'")
            return bpf
        ...
        if not bpf:
            ...
            logger.warning(
                f"Mode '{mode_name.value}' returned empty BPF filter! "
                f"Using emergency fallback: '{fallback}'"
            )
            return fallback

        logger.info(f"Mode '{mode_name.value}': BPF filter = '{bpf}'")
        return bpf
    ...
    def validate_bpf_syntax(self) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                logger.error(
                    f"BPF filter syntax error: {result.stderr.strip()}"
                )
                return False
            return True
        except Exception as exc:
            logger.debug(f"BPF validation failed: {exc}")
