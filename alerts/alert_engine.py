...
class AlertEngine:
    ...
    # MAC whitelist — devices in this set will NOT trigger security alerts.
    # Populated at startup from config or a whitelist file.
    _mac_whitelist: set = set()
    _known_macs: set = set()

    @classmethod
    def load_mac_whitelist(cls, macs: list):
        """Load a list of trusted MAC addresses (case-insensitive)."""
        cls._mac_whitelist = {m.lower().replace("-", ":") for m in macs if m}
        logger.info("MAC whitelist loaded: %d entries", len(cls._mac_whitelist))

    @classmethod
    def add_known_mac(cls, mac: str):
        """Mark a MAC as known (won't trigger future alerts)."""
        if mac:
            cls._known_macs.add(mac.lower().replace("-", ":"))
