...
Verifies that:
- ``_on_mode_change()`` properly stops the old engine and starts a new one
- The ``_engine_lock`` prevents race conditions
...
class TestModeChangeRestart:
    """Integration tests for _on_mode_change."""
    ...
    def test_engine_stopped_on_mode_change(self):
        ...
        new_mode = _make_mode("wifi_client", ip="192.168.1.50", iface="wlan0")

        with patch.object(main, '_capture_engine', old_engine), \
    ...
    def test_disconnect_stops_engine(self):
        ...
        with patch.object(main, '_capture_engine', old_engine), \
             patch.object(main, '_expose_engine_to_routes'):
            main._on_mode_change(old_mode, disconnected_mode)
    ...
    def test_engine_lock_prevents_concurrent_mutation(self):
        ...
        original_locked = main._on_mode_change_locked

        def slow_locked(old, new, logger):
            call_order.append('enter')
            time.sleep(0.1)
            call_order.append('exit')

        old_mode = _make_mode("ethernet")
        new_mode1 = _make_mode("wifi_client", ip="192.168.1.50")
        new_mode2 = _make_mode("hotspot", ip="192.168.137.1")

        with patch.object(main, '_on_mode_change_locked', side_effect=slow_locked):
            t1 = threading.Thread(target=main._on_mode_change, args=(old_mode, new_mode1))
            t2 = threading.Thread(target=main._on_mode_change, args=(old_mode, new_mode2))
            t1.start()
    ...
    def test_expose_engine_called_after_restart(self):
        """Flask routes should see the new engine via _expose_engine_to_routes."""
        import main
        old_engine = MagicMock()
        old_engine.is_running = True
        ...
        with patch.object(main, '_capture_engine', old_engine), \
             patch.object(main, '_create_capture_engine') as mock_create, \
             patch.object(main, '_expose_engine_to_routes') as mock_expose, \
             patch('main.time'), \
             patch.dict('sys.modules', {'psutil': MagicMock()}):
    ...
    def test_subnet_reset_on_mode_change(self):
        ...
        old_mode = _make_mode("ethernet", ip="192.168.1.100")
        new_mode = _make_mode("wifi_client", ip="10.0.0.50", iface="wlan0")
