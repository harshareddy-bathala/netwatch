...
def rollup_traffic(raw_retention_hours: int = 24) -> dict:
    ...
    try:
        with get_connection() as conn:
            ...
            cursor.execute("""
                INSERT OR REPLACE INTO traffic_rollup
                ...
                SELECT
                    ...
                    COALESCE(
                        (SELECT total_bytes FROM traffic_rollup r
                         WHERE r.hour_bucket = strftime('%Y-%m-%d %H:00:00', t.timestamp)
                           AND r.source_ip = t.source_ip
                           AND r.dest_ip = t.dest_ip
                           AND r.protocol = t.protocol
                           AND r.direction = COALESCE(t.direction, 'unknown')),
                        0
                    ) + SUM(bytes_transferred),
                    COALESCE(
                        (SELECT packet_count FROM traffic_rollup r
                         WHERE r.hour_bucket = strftime('%Y-%m-%d %H:00:00', t.timestamp)
                           AND r.source_ip = t.source_ip
                           AND r.dest_ip = t.dest_ip
                           AND r.protocol = t.protocol
...
def cleanup_old_rollups(days_to_keep: int = 90) -> int:
    """Delete rollup rows older than *days_to_keep* days and reclaim disk space."""
    ...
    try:
        with get_connection() as conn:
            ...
            if deleted:
                ...
                # Reclaim disk space periodically after large deletions
                if deleted > 1000:
                    try:
                        conn.execute("VACUUM")
                        logger.info("Database VACUUM completed after large cleanup")
