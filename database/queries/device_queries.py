...
def deactivate_stale_devices(new_subnet_prefix: str) -> int:
    """
    Mark devices that do NOT belong to *new_subnet_prefix* as stale.

    Called during mode changes so that ghost devices from the previous
    subnet stop appearing in the active device list.  Their ``last_seen``
    is pushed 10 minutes into the past, which is beyond the default
    active-device window (5 min).  The records are NOT deleted — they
    will reappear if the user switches back and the device is seen again.
    ...
    try:
        with get_connection() as conn:
            cursor = conn.cursor()
            stale_time = (datetime.now() - timedelta(minutes=10)).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            cursor.execute(
                """
                ...
                WHERE ip_address NOT LIKE ?
                  ...
                  AND ip_address != ''
                """,
                (stale_time, f"{new_subnet_prefix}.%"),
            )
            conn.commit()
            count = cursor.rowcount
            if count:
                logger.info(
                    "Deactivated %d device(s) not in subnet %s.*",
                    count, new_subnet_prefix,
                )
            return count
    except sqlite3.Error as e:
        logger.error("deactivate_stale_devices error: %s", e)
        return 0
...
def get_active_device_count(minutes: int = 5, conn=None) -> int:
    ...
    **SINGLE SOURCE OF TRUTH** for the active-device count.
    ...
    try:
        if conn is not None:
            # Reuse the caller-provided connection
            return _do_count(conn.cursor())
        else:
            with get_connection() as new_conn:
                return _do_count(new_conn.cursor())
    except sqlite3.Error as e:
...
def get_active_devices(minutes: int = 5, limit: int = 100) -> list:
    ...
    try:
        with get_connection() as conn:
            ...
            # Build subnet filter — skip in port_mirror/hotspot modes
...
def get_all_devices(limit: int = 100, offset: int = 0, hours: int = 24) -> list:
    ...
    Includes subnet filtering to only show devices on current network.
    In port_mirror/hotspot modes, subnet filtering is skipped.
    Also merges in devices from the ``devices`` table (populated by ARP
    ...
    try:
        with get_connection() as conn:
            ...
            cursor.execute(f"""
                ...
                ORDER BY total_bytes DESC
                LIMIT ? OFFSET ?
            """, (*params_src, *params_dst, *params_dev, limit * 3, offset))
            ...
            # Deduplicate by IP so MAC-randomised devices don't appear twice
            # Over-fetched (limit*3) so dedup doesn't shrink below the requested page size
            deduped = _deduplicate_devices_by_ip(devices)
